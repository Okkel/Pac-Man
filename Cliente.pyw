...
#from Client_check_point import Mensagem
...
class game ():
    ...
    def defaulthiscorelist(self):
            return [ (100000,"Jao") , (80000,"Arthur") , (60000,"Count Pacula") ]
    ...
    def AddToScore (self, amount,flag = 1):

        # Nao ganhara vida extra
    ...
    def SmartMoveScreen (self):
        ...
        possibleScreenX = playerServer.x - self.screenTileSize[1] / 2 * 16
        possibleScreenY = playerServer.y - self.screenTileSize[0] / 2 * 16
    ...
    def SetNextLevel (self):
        ...
        playerServer.velX = 0
        playerServer.velY = 0
        playerServer.anim_pacmanCurrent = playerServer.anim_pacmanS
    ...
    def Draw (self):
        ...
        for y in range(4, 8, 1):
            for x in range(3, 7, 1):
                ...
                if player.x > self.x and player.y > self.y :#or playerServer.x > self.x and playerServer.y > self.y:
                    #player is to lower-right
                    pupilSet = (5, 6)
                elif player.x < self.x and player.y > self.y :#or playerServer.x < self.x and playerServer.y > self.y:
                    ...
                    pupilSet = (3, 4)
                elif playerServer.x > self.x and playerServer.y > self.y :#or playerServer.x > self.x and playerServer.y > self.y:
                    #player is to lower-right
                    pupilSet = (5, 6)
                elif playerServer.x < self.x and playerServer.y > self.y :#or playerServer.x < self.x and playerServer.y > self.y:
                    #player is to lower-left
    ...
    def Move (self):
        ...
        if (self.x % 16) == 0 and (self.y % 16) == 0:
            ...
            if (self.currentPath):
                ...
                self.currentPath = path.FindPath( (self.nearestRow, self.nearestCol), (playerServer.nearestRow, playerServer.nearestCol) )
                self.FollowNextPathWay()
...
class pacman ():
    ...
    def Move (self):
        ...
        #PLAYER REDE RECEBE AQUI O MOVIMENTO

        if not thisLevel.CheckIfHitWall((self.x + self.velX, self.y + self.velY), (self.nearestRow, self.nearestCol)):
            # it's ok to Move
            self.x += self.velX
            self.y += self.velY
            #ENVIAR PRO SERVIDOR ESTE MOVIMENTO
            # check for collisions with other tiles (pellets, etc)
            thisLevel.CheckIfHitSomething((self.x, self.y), (self.nearestRow, self.nearestCol),0)
...
#           self.anim_pacmanS[i] = pygame.image.load(os.path.join(SCRIPT_PATH,"res","sprite","pacmann.gif")).convert()
    ...
    def Move (self):
        ...
        if not thisLevel.CheckIfHitWall((self.x + self.velX, self.y + self.velY), (self.nearestRow, self.nearestCol)):
            ...
            #ENVIAR PRO SERVIDOR ESTE MOVIMENTO
            # check for collisions with other tiles (pellets, etc)
            thisLevel.CheckIfHitSomething((self.x, self.y), (self.nearestRow, self.nearestCol),1)
...
class level ():
    ...
    def CheckIfHitSomething (self, (playerX, playerY), (row, col),flag):#se flag ==1 => movimento do serverpacman e portanto o score nao deve ser alterado
                                                                        #se flag ==0 => movimento do pacman(player) portanto o score e alterado
        for iRow in range(row - 1, row + 2, 1):
            for iCol in range(col - 1, col + 2, 1):
                ...
                if isReadingLevelData == True:
                    ...
                    for k in range(0, self.lvlWidth, 1):
                        ...
                        if thisID == 4:
                            ...
                            playerServer.homeX = k * 16
                            playerServer.homeY = rowNum * 16
...
#        thisGame.fruitTimer = 0
        ...
        player.anim_pacmanCurrent = player.anim_pacmanS
        player.animFrame = 3
        playerServer.x = playerServer.homeX
        playerServer.y = playerServer.homeY
        playerServer.velX = 0
        playerServer.velY = 0

        playerServer.anim_pacmanCurrent = playerServer.anim_pacmanS
        playerServer.animFrame = 3
...
def CheckInputs():
    ...
    if thisGame.mode == 1:
        ...
        #     #player rede
        #     if not thisLevel.CheckIfHitWall((playerServer.x + playerServer.speed, playerServer.y), (playerServer.nearestRow, playerServer.nearestCol)):
        #         playerServer.velX = playerServer.speed
        #         playerServer.velY = 0
        #
        # elif (int(m.msg) == 2):# or pygame.key.get_pressed()[ pygame.K_a ] :
        #     if not thisLevel.CheckIfHitWall((playerServer.x - playerServer.speed, playerServer.y), (playerServer.nearestRow, playerServer.nearestCol)):
        #         playerServer.velX = -playerServer.speed
        #         playerServer.velY = 0
        #
        #
        # elif (int(m.msg) == 3) : #or pygame.key.get_pressed()[ pygame.K_s ]  :
        #     if not thisLevel.CheckIfHitWall((playerServer.x, playerServer.y + playerServer.speed), (playerServer.nearestRow, playerServer.nearestCol)):
        #         playerServer.velX = 0
        #         playerServer.velY = playerServer.speed
        #
        # elif (int(m.msg)== 4) : #or pygame.key.get_pressed()[ pygame.K_w ] :
        #     if not thisLevel.CheckIfHitWall((playerServer.x, playerServer.y - playerServer.speed), (playerServer.nearestRow, playerServer.nearestCol)):
        #         playerServer.velX = 0
        #         playerServer.velY = -playerServer.speed
...
def CheckInputsServer():
    ...
    if thisGame.mode == 1:
        #####################INPUTS DO SERVER##############################
        if (int(m.msg) == 1) or pygame.key.get_pressed()[ pygame.K_d ]  :
            #player rede
            if not thisLevel.CheckIfHitWall((playerServer.x + playerServer.speed, playerServer.y), (playerServer.nearestRow, playerServer.nearestCol)):
                playerServer.velX = playerServer.speed
                playerServer.velY = 0
    ...
    def recebe(self,a,b):
        while True:
            ...
            #     return msg
                ...
                thisGame.SetMode(3)
                self.adScore = tcp.recv(64)
                #print "adversario" ,self.msg
                self.envia(thisGame.AddToScore(0,0))
                # #b = m.recebe()
                # b = int(m.msg)
                # sys.exit()
                break
...
#         HOST = ip  # HOST = '172.16.254.209'# Endereco IP do Servidor
...
HOST = gui.ip  # HOST = '172.16.254.209'# Endereco IP do Servidor
PORT =  int(gui.porta)# Porta que o Servidor esta
...
while True:
    ...
    elif thisGame.mode == 6:#confira
        # pause after eating all the pellets
        thisGame.modeTimer = 3
        thisGame.SetMode( 3 )
    ...
    if thisGame.mode == 5:
        ...
        thisGame.DrawNumber (thisGame.ghostValue / 2, (playerServer.x - thisGame.screenPixelPos[0] - 4, playerServer.y - thisGame.screenPixelPos[1] + 6))
